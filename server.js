...
app.use(
  cors({
    origin: ["https://pluggstugan.netlify.app"], // ✅ your frontend
...
const io = new Server(server, {
  cors: {
    origin: ["https://pluggstugan.netlify.app"], // ✅ allow Netlify frontend
    methods: ["GET", "POST", "DELETE"],
    credentials: true,
  },
});

app.use(express.static(path.join(__dirname, "public"))); // ✅ optional: static serving if needed
...
io.on("connection", (socket) => {
  ...
  socket.on("chatMessage", ({ room, user, message }) => {
    ...
    const msg = { user, message };
    if (!chatrooms[room])
      chatrooms[room] = { name: room, messages: [] };
    chatrooms[room].messages.push(msg);
    io.to(room).emit("chatMessage", msg);
  });
  ...
  socket.on("disconnect", () => {
    const user = Object.keys(userSockets).find(
      (u) => userSockets[u] === socket.id
    );
    if (user) {
      console.log(`❌ ${user} disconnected`);
      delete userSockets[user];
      delete userRooms[user];
      loggedInUsers.delete(user);
      io.emit("updateOnlineUsers", getOnlineStatus());
    }
  });
});
